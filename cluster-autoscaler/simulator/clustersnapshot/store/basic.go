...
func (data *internalBasicSnapshotData) listNodeInfos() []fwk.NodeInfo {
	...
	for _, v := range data.nodeInfoMap {
		nodeInfoList = append(nodeInfoList, v.ToScheduler())
	}
	return nodeInfoList
}
...
func (data *internalBasicSnapshotData) listNodeInfosThatHavePodsWithAffinityList() ([]fwk.NodeInfo, error) {
	...
	for _, v := range data.nodeInfoMap {
		if len(v.GetPodsWithAffinity()) > 0 {
			havePodsWithAffinityList = append(havePodsWithAffinityList, v.ToScheduler())
		}
	}
...
func (data *internalBasicSnapshotData) listNodeInfosThatHavePodsWithRequiredAntiAffinityList() ([]fwk.NodeInfo, error) {
	...
	for _, v := range data.nodeInfoMap {
		if len(v.GetPodsWithRequiredAntiAffinity()) > 0 {
			havePodsWithRequiredAntiAffinityList = append(havePodsWithRequiredAntiAffinityList, v.ToScheduler())
		}
	}
...
func (data *internalBasicSnapshotData) addPod(podInfo *framework.PodInfo, nodeName string) error {
	if _, found := data.nodeInfoMap[nodeName]; !found {
		return clustersnapshot.ErrNodeNotFound
	}
	data.nodeInfoMap[nodeName].AddPod(podInfo.DeepCopy())
...
func (snapshot *BasicSnapshotStore) StoreNodeInfo(nodeInfo *framework.NodeInfo) error {
	...
	data.nodeInfoMap[nodeInfo.Node().Name] = nodeInfo.DeepCopy()
...
func (snapshot *BasicSnapshotStore) SetClusterState(nodes []*apiv1.Node, scheduledPods []*apiv1.Pod, draSnapshot *drasnapshot.Snapshot) error {
	...
	for _, pod := range scheduledPods {
		if knownNodes[pod.Spec.NodeName] {
			...
			if err := snapshot.getInternalData().addPod(podInfo, pod.Spec.NodeName); err != nil {
				return err
			}
		}
	}
...
// ForceRemoveNodeInfo removes nodes (and pods scheduled to it) from the snapshot.
func (snapshot *BasicSnapshotStore) ForceRemoveNodeInfo(nodeName string) error {
...
func (snapshot *BasicSnapshotStore) StorePodInfo(podInfo *framework.PodInfo, nodeName string) error {
	return snapshot.getInternalData().addPod(podInfo, nodeName)
}
