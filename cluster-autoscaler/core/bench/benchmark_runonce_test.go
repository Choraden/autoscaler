...
func setupScaleDown60Percent(nodesCount int) func(*integration.FakeSet) error {
	return func(cluster *integration.FakeSet) error {
		...
		for i := range nodesCount {
			...
			SetNodeReadyState(node, true, time.Now().Add(-24*time.Hour))
			if _, err := cluster.KubeClient.CoreV1().Nodes().Create(context.Background(), node, metav1.CreateOptions{}); err != nil {
				return err
			}
			cluster.CloudProvider.AddNode(ngName, node)
		}
		...
		for i := range nodesCount * 40 {
			podName := fmt.Sprintf("pod-%d", i)
			nodeName := fmt.Sprintf("n-%d", i%nodesCount)
